...
export default function Navbar({ className = "", onMobileMenuChange }: NavbarProps) {
        ...
        <div className="relative flex min-h-12 w-full items-center justify-between gap-4 lg:min-h-14">
            ...
            <ul className="flex items-center gap-7 xl:gap-9">
              {NAV_LINKS.map((link, index) => (
                ...
                >
                  <Link
                    href={link.href}
                    className="text-[15px] font-normal text-White transition-opacity hover:opacity-70"
                  >
                    {link.label}
                  </Link>
                </motion.li>
          ...
          <div className="relative z-10 flex items-center gap-3">
            ...
            >
              <Link
                href="#contact"
                className="inline-flex items-center gap-2.5 text-[15px] text-White transition-opacity hover:opacity-70"
              >
                <Image
                  src="/logo/phone.svg"
                  alt=""
